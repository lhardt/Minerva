...
DescTimetablePane::DescTimetablePane(Application * owner, wxWindow * parent, wxPoint pos) : wxScrolledWindow(parent, wxID_ANY, pos, wxSize(600,400), wxSIMPLE_BORDER){
	...
	if(school->all_meetings != NULL){
		...
		wxStaticText * warning = new wxStaticText(this, wxID_ANY, wxT("Ops... Parece que você não gerou nenhum horário ainda!"), wxDefaultPosition, wxSize(500,20));

		sizer->Add(title, 0, wxALL | wxALIGN_CENTER, 15);
		sizer->Add(warning, 0, wxALL,15);
	}
